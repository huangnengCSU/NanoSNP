...
def extract_pileups_group(bam, groups, chromosome_high_confident_variants, max_coverage):
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=total_threads) as executor:
            if len(divided_groups) == total_threads:
                signals = [executor.submit(extract_pileups_group, args.bam, divided_groups[thread_id],
                                           chromosome_high_confident_variants, args.max_coverage) for thread_id in
                           range(0, total_threads)]
            else:
                # 如果chromosome_groups恰好被total_threads整除，则divided_groups长度只有total_threads-1，会出现数组越界错误。
                signals = [executor.submit(extract_pileups_group, args.bam, divided_groups[thread_id],
                                           chromosome_high_confident_variants, args.max_coverage) for thread_id in
                           range(0, len(divided_groups))]
