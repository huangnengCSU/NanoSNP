...
def extract_pileups_group(bam, groups, chromosome_high_confident_variants, max_coverage, pileup_length):
    ctgname = groups[0][0].ctgname
    out_cand_pos, out_cand_edge_columns, out_cand_edge_matrix, out_cand_pair_columns, out_cand_pair_route, out_labels = [], [], [], [], [], []
    ...
    return out_cand_pos, out_cand_edge_columns, out_cand_edge_matrix, out_cand_pair_columns, out_cand_pair_route, \
           out_labels, out_cand_group_pos, out_cand_read_matrix, out_cand_baseq_matrix, out_cand_mapq_matrix, \
           max_depth, out_cand_surrounding_read_matrix, out_cand_surrounding_baseq_matrix, \
...
def Run(args):
    ...
    for k in groups_dict.keys():
        out_cand_pos, out_cand_edge_columns, out_cand_edge_matrix, out_cand_pair_columns, out_cand_pair_route, out_cand_labels = [], [], [], [], [], []
        out_cand_group_pos, out_cand_read_matrix, out_cand_baseq_matrix, out_cand_mapq_matrix = [], [], [], []
        out_cand_surrounding_read_matrix, out_cand_surrounding_baseq_matrix, out_cand_surrounding_mapq_matrix = [], [], []
        ...
        with concurrent.futures.ProcessPoolExecutor(max_workers=total_threads) as executor:
            if len(divided_groups) == total_threads:
                signals = [executor.submit(extract_pileups_group, args.bam, divided_groups[thread_id],
                                           chromosome_high_confident_variants, args.max_coverage, pileup_length) for
            ...
            for sig in concurrent.futures.as_completed(signals):
                if sig.exception() is None:
                    # get the results
                    [cand_pos, cand_edge_columns, cand_edge_matrix, cand_pair_columns, cand_pair_route, cand_labels,
                     cand_group_pos, cand_read_matrix, cand_baseq_matrix, cand_mapq_matrix, depth,
                     cand_surrounding_read_matrix, cand_surrounding_baseq_matrix,
                    ...
                    if len(cand_edge_matrix) > 0:
                        ...
                        out_cand_pair_columns.extend(cand_pair_columns)
                        out_cand_pair_route.extend(cand_pair_route)
                        out_cand_labels.extend(cand_labels)
                        out_cand_group_pos.extend(cand_group_pos)
                        out_cand_read_matrix.extend(cand_read_matrix)
                        out_cand_baseq_matrix.extend(cand_baseq_matrix)
                        out_cand_mapq_matrix.extend(cand_mapq_matrix)
            ...
            # 排序
        out_cand_pos = np.array(out_cand_pos)
        ...
        out_cand_labels = np.array(out_cand_labels)[new_index]
        ...
        table_file.create_earray(
            where='/', name='surrounding_base_quality_matrix', atom=int_atom, shape=[0, max_surrounding_depth, pileup_length])
        table_file.create_earray(
            where='/', name='surrounding_mapping_quality_matrix', atom=int_atom, shape=[0, max_surrounding_depth, pileup_length])
        ...
        table_file.create_earray(where='/', name='group_positions', atom=string_atom, shape=(0, adjacent_size * 2 + 1),
                                 filters=TABLE_FILTERS)
        table_file.create_earray(
            where='/', name='labels', atom=int_atom, shape=[0, 3])
        ...
        table_file.root.labels.append(np.array(out_cand_labels).reshape(-1, 3))
...
def main():
    parser = ArgumentParser(description="Group adjacent SNPs for training")
    ...
    parser.add_argument("--min_quality", type=int, default=15,
                        help="The minimum quality of snp site used as edge model.")
