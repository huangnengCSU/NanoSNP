...
class LoadDataset(Dataset):
    def __init__(self, data_dir1, data_dir2, output, max_depth=20, min_depth=5):
    ...
    def __getitem__(self, i):
        ...
        g1_tag1_max = np.max(g1_tag1_base_percentage[2], axis=1)  # [N]
        g1_tag1_argmax = np.argmax(g1_tag1_base_percentage[2], axis=1)  # [A,C,G,T,D]

        g1_tag2_max = np.max(g1_tag2_base_percentage[2], axis=1)
        g1_tag2_argmax = np.argmax(g1_tag2_base_percentage[2], axis=1)
        ...
        for p in np.array(positions1)[idx1][filtered_idx][mask==False]:
            self.fout.write(p+'\n')

        table_file1.close()
        table_file2.close()

        return g0, g1, g2, g3, gt_label
...
if __name__ == '__main__':
    ...
    train_dataset = LoadDataset(opt.tag_bin1, opt.tag_bin2, opt.output)
