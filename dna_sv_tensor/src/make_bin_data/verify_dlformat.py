...
def main(argv):
    ...
    table_file = tables.open_file(binary_tensor_input, 'r')
    label_list = table_file.root.label 
    gt_label_list = np.array(label_list[:, :21].argmax(1))
    print(gt_label_list.shape)

    N_LABEL = len(GT21_LABLES)
    for i in range(N_LABEL):
        cnt = sum(gt_label_list == i)
        print('cnt[%s]\t = %d' % (GT21_LABLES[i], cnt))
