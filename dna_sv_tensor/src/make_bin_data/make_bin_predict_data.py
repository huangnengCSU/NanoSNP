...
def write_table_dict(table_dict, position_matrix, pos, alt_info):
...
def transform_one_input(input, tensor_shape, float_type, table_dict, table_file, total_transformed_tensor):
    ...
    for line in open(input, "r"):
        ...
        if len(columns) < 3:
            ...
            sys.exit(1)
        position_matrix, position, alt_info = columns[0], columns[1], columns[2]
        #print("label = %s, len = %d" % (label, len(label)))
        write_table_dict(table_dict, position_matrix, position, alt_info)
...
def main(argv):
    ...
    for i in range(2, n_param):
        ...
        total_transformed_tensor = transform_one_input(argv[i], tensor_shape, float_type, table_dict, table_file, total_transformed_tensor)
        print('Done.')
